...
const MobileDatePicker = ({ date, onSelect }: { date: Date | undefined; onSelect: (d: Date | undefined) => void }) => {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          className={`w-full h-14 justify-between font-bold font-assistant rounded-xl border-white/10 bg-white/5 gap-3 ${!date && "text-white/30"}`}
        >
          <span>{date ? format(date, "PPP", { locale: he }) : "בחר תאריך יעד"}</span>
          <CalendarIcon className="icon-md text-primary opacity-80 shrink-0" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-screen max-w-[calc(100vw-2rem)] p-0 bg-card border-white/10 rounded-2xl overflow-hidden z-[9999]" align="center">
        <Calendar mode="single" selected={date} onSelect={(d) => { onSelect(d); setOpen(false); }} locale={he} />
      </PopoverContent>
    </Popover>
  );
};
...
const pillBase = "px-4 py-2 rounded-full text-xs font-bold font-assistant border transition-all duration-200";
const pillActive = "bg-primary border-primary text-white shadow-glow-primary scale-105";
const pillIdle   = "bg-white/5 border-white/5 text-white/60 hover:bg-white/10 hover:border-white/10";
const pillActiveAccent = "bg-accent border-accent text-white shadow-lg scale-105";
...
export const FilterPanel = ({
  ...
  const activeFiltersCount = [statusFilter !== "all", departmentFilter !== "all", !!dateFilter].filter(Boolean).length;
    ...
    <>
      ...
      <div className="hidden lg:block glass-card p-6 mb-8 animate-fade-in">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold font-rubik">מרכז סינון מתקדם</h2>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="relative group">
              <Search className="icon-sm absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground group-focus-within:text-primary transition-colors" />
              <input
            ...
            <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
              <PopoverTrigger asChild>
                <Button variant="outline"
                  className={`h-12 justify-between font-assistant rounded-xl border-white/10 bg-white/5 hover:bg-white/10 transition-all gap-3 ${!dateFilter && "text-muted-foreground"}`}>
                  <span>{dateFilter ? format(dateFilter, "PPP", { locale: he }) : "סינון לפי תאריך"}</span>
                  <CalendarIcon className="icon-sm text-primary shrink-0" />
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0 bg-card border-white/10 shadow-2xl" align="end">
                <Calendar mode="single" selected={dateFilter}
                  ...
                  locale={he} disabled={(d) => isFuture(d) || isFriday(d) || isSaturday(d)} />
              </PopoverContent>
            </Popover>
          </div>
              ...
              <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto p-1 custom-scrollbar">
                {DEPT_OPTIONS.map((dept) => (
                  <button key={dept} onClick={() => setDepartmentFilter(dept)}
                    className={`${pillBase} ${departmentFilter === dept ? pillActiveAccent : pillIdle}`}>
      ...
      <div className="lg:hidden flex gap-3 mb-6">
        <div className="relative flex-1 group">
          <Search className="icon-sm absolute right-4 top-1/2 -translate-y-1/2 text-white/40" />
        ...
        <Button variant="outline"
          className={`h-12 w-12 rounded-xl p-0 relative border-white/10 shrink-0 transition-all ${activeFiltersCount > 0 ? "bg-primary/20 border-primary/40 text-primary" : "bg-white/5"}`}
        ...
        <div className="fixed inset-0 z-[60] lg:hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-card border-t border-white/10 rounded-t-[2.5rem] max-h-[85vh] flex flex-col shadow-2xl animate-in slide-in-from-bottom-full duration-500 ease-out">
            ...
            <div className="p-6 border-b border-white/5 flex items-center justify-between sticky top-0 bg-card/80 backdrop-blur-xl z-10">
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-lg bg-primary/10 text-primary shrink-0"><Filter className="icon-md" /></div>
            ...
            {/* Body */}
            <div className="p-6 space-y-8 overflow-y-auto pb-40 custom-scrollbar">
              {/* Status */}
              <div className="space-y-3">
                  ...
                  {STATUS_OPTIONS.map((s) => (
                    <button key={s.id} onClick={() => setStatusFilter(s.id)}
                      className={`h-12 rounded-xl text-sm font-bold font-assistant border flex items-center justify-center gap-2 transition-all ${
                        statusFilter === s.id ? "bg-primary border-primary text-white shadow-glow-primary" : "bg-white/5 border-white/5 text-white/60 active:bg-white/10"
                      }`}>
                      {statusFilter === s.id && <Check className="icon-sm" />}
                      {s.label}
                    </button>
                  ))}
                </div>
                  ...
                  {DEPT_OPTIONS.map((dept) => (
                    <button key={dept} onClick={() => setDepartmentFilter(dept)}
                      className={`h-12 rounded-xl text-xs font-bold font-assistant border flex items-center justify-center gap-2 transition-all ${
                        departmentFilter === dept ? "bg-accent border-accent text-white shadow-lg" : "bg-white/5 border-white/5 text-white/60 active:bg-white/10"
                      }`}>
                      {departmentFilter === dept && <Check className="icon-xs" />}
                      {dept === "all" ? "כל המדורים" : dept}
                    </button>
                  ))}
                </div>
            ...
            {/* Sticky actions */}
            <div className="p-6 border-t border-white/10 bg-card/90 backdrop-blur-xl sticky bottom-0 flex gap-4 z-10 safe-bottom">
              <Button variant="ghost" className="flex-1 font-bold font-assistant h-14 rounded-xl hover:bg-white/5"
                onClick={() => { onClearFilters?.(); setIsMobileFilterOpen(false); }}>
                נקה הכל
              </Button>
              <Button className="flex-[2] font-bold font-assistant h-14 rounded-xl bg-primary text-white shadow-glow-primary active:scale-95 transition-transform"
                onClick={() => setIsMobileFilterOpen(false)}>
